...
import {
  getTrackingHeaders as getClientTrackingHeaders,
  collectTrackingResponseHeaders as collectClientTrackingResponseHeaders,
} from "./analytics/trackingContext";

async function getServerTrackingModule() {
  if (!import.meta.env.SSR) return undefined;
  return import("./analytics/trackingContext.server");
}

function getApiUrl(): string {
  const isSSR = import.meta.env.SSR;
  if (isSSR) {
    return `https://${config.shopifyShop}/api/${config.apiVersion}/graphql.json`;
  }
  return `/api/${config.apiVersion}/graphql/`;
}
...
export async function makeShopifyRequest<T = any>(
  ...
  async function getOptions(): Promise<RequestInit> {
    if (isSSR && !buyerIP) {
      console.error(
    ...
    if (isSSR) {
      headers["X-Shopify-Storefront-Access-Token"] = privateShopifyAccessToken;
      if (buyerIP) {
        headers["Shopify-Storefront-Buyer-IP"] = buyerIP;
      }
      // Inject tracking tokens generated by middleware (server-only module)
      const trackingModule = await getServerTrackingModule();
      const trackingHeaders = trackingModule ? trackingModule.getTrackingHeaders() : getClientTrackingHeaders();
  ...
  const response = await fetch(apiUrl, await getOptions());

  if (isSSR) {
    const trackingModule = await getServerTrackingModule();
    if (trackingModule) {
      trackingModule.collectTrackingResponseHeaders(response);
    } else {
      collectClientTrackingResponseHeaders(response);
    }
  }
